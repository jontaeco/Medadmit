...
export function SankeyDiagram({
  ...
  const sankeyData = useMemo(() => {
    ...
    modalOutcome.schoolsWithAcceptance.forEach((schoolId) => {
    ...
    const rejectedPostIISchoolIds = modalOutcome.schoolsWithInterview.filter(
      (id) => !modalOutcome.schoolsWithAcceptance.includes(id)
    )
    ...
    allSchools.forEach((school, idx) => {
      const gotInterview = modalOutcome.schoolsWithInterview.includes(school.school.id)
      if (gotInterview) {
    ...
    return { nodes, links }
  }, [schoolList, modalOutcome])

  // Calculate totals for display
  const totalSchools =
    schoolList.reach.length + schoolList.target.length + schoolList.safety.length
  const interviewCount = modalOutcome.schoolsWithInterview.length
  const acceptedCount = modalOutcome.schoolsWithAcceptance.length
    ...
    <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
      ...
      <Card className="lg:col-span-3">
        <CardHeader>
          ...
          <CardDescription>
            Most likely outcome from Monte Carlo simulation ({(modalOutcome.frequency * 100).toFixed(1)}% of simulations)
          </CardDescription>
        </CardHeader>
        <CardContent>
          ...
          <div className="grid grid-cols-3 gap-4 mt-6 pt-6 border-t">
            ...
            <div className="text-center">
              <p className="text-2xl font-bold text-amber-600">{interviewCount}</p>
      ...
      <Card className="lg:col-span-1">
        ...
        <CardContent className="space-y-4 text-sm">
          ...
          <div className="pt-4 border-t">
            <p className="font-medium text-slate-500 dark:text-slate-400 mb-2">Modal Outcome</p>
            <div className="space-y-1">
              <div className="flex justify-between">
                <span>Interviews</span>
                <span className="font-medium text-amber-600">{interviewCount}</span>
              </div>
              <div className="flex justify-between">
